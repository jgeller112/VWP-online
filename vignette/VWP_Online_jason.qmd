...
edatibs1 <- edatibs %>%
  ...
  select(-"NA") %>%
  filter(face_conf >= .5, acc>=.8, Experiment.Version.x==10)
...
vwp_files<-list.files(here::here("jason", "target", "uploads"), pattern = ".xlsx") # get files 
agg_eye_data <- read.csv(here::here("jason","target",  "data_exp_77588-v12_task-8vmc.csv"))
...
#| echo: true
# merge all data files (gorilla stores each trial as one file for each p)
setwd(here::here("jason","target",  "uploads"))
